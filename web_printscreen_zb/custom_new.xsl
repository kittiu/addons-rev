...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:import href="../web_printscreen_zb/custom_default_printscreen.xsl"/>
	<xsl:import href="../web_printscreen_zb/custom_rml_printscreen.xsl"/>
	<xsl:template match="/">
	...
	<xsl:template name="stylesheet">
		<paraStyle name="title" fontName="THSarabun Bold" fontSize="22" alignment="center"/>
	...
	<xsl:template match="header">
		...
		<xsl:for-each select="field">
			<td>
			<para style="tbl_heading"><font fontName="THSarabun Bold" fontSize="9">
					...
					<xsl:choose>
						<xsl:when test="@tree='yes'">
							<para style="test">
								...
								<font fontName="THSarabun" fontSize="9">
								<xsl:value-of select="."/>
								</font>
							</para>
						</xsl:when>
                       <xsl:when test="@tree='float'">
                           <para style="float_right"><font fontName="THSarabun" fontSize="9">
                               <xsl:value-of select="."/>
                               </font></para>
                       </xsl:when>
				...
				<xsl:when test="@para='group'">
					<xsl:choose>
						<xsl:when test="@tree='yes'">
							<para>
								<xsl:attribute name="leftIndent"><xsl:value-of select="@space"/></xsl:attribute>
								<font fontName="THSarabun Bold" fontSize="9">
								<xsl:value-of select="."/>
								</font>
							</para>
						</xsl:when>
                       <xsl:when test="@tree='float'">
                           <para style="float_right"><font fontName="THSarabun Bold" fontSize="9" color="black">
                               <xsl:value-of select="."/>
                               </font></para>
                       </xsl:when>
                       <xsl:when test="@tree='undefined'">
                            <para>
                                ...
                                <font fontName="THSarabun Bold" fontSize="9" color="gray">
                                    <xsl:value-of select="."/>
                                </font>
                            </para>
                       </xsl:when>
						...
						<xsl:otherwise>
							<para>
								<font fontName="THSarabun Bold" fontSize="9" color="black">
								<xsl:value-of select="."/>
								</font>
							</para>
						</xsl:otherwise>
